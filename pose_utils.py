...
def cords_to_map(cords, img_size, old_size=(128, 64), affine_matrix=None, sigma=6,misv=-1):
    old_size = img_size if old_size is None else old_size
    cords = cords.astype(float)
    result = np.zeros(img_size + cords.shape[0:1], dtype='float32')
    for i, point in enumerate(cords):
        if point[0] == misv or point[1] == misv:
            continue
...
def draw_pose_from_cords(array, img_size, old_size=(128, 64), radius=2, draw_bones=True,misv=-1):
    ...
    if draw_bones:
        for i, (f, t) in enumerate(BONES):
            from_missing = array[f][0] == misv or array[f][1] == misv
            to_missing = array[t][0] == misv or array[t][1] == misv
    ...
    for i, joint in enumerate(array):
        if array[i][0] == misv or array[i][1] == misv:
            continue
